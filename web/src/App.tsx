  ...
  const UIButton: React.FC<{ uis: UINodeBase[] }> = ({ uis }) => {
    ...
    return(
      <div className="absolute z-10 bottom-8 inset-x-0 text-center">
        {buttonJSXs.map((ui, index) => <span key={index} className='[&>div>div>button]:px-4 [&>div>div>span]:hidden [&>div>div>button]:py-2 [&>div>div>button]:bg-content-dark [&>div>div>button]:rounded-sm [&>div>div>button]:text-white'>{ui}</span>)}
      </div>
    )
    
  }

  const UISlider: React.FC<{ uis: UINodeBase[] }> = ({ uis }) => {
    const sliderUiNodes = uis.filter(ui => ui.label === 'width'|| ui.label === 'dimension'|| ui.label === 'height');
    ...
    return(
      <div className="absolute z-10 bottom-20 inset-x-0 text-center">
        {sliderJSXs.map((ui, index) => <span key={index} className='[&>div>div>input]:range'>{ui}</span>)}
      </div>
