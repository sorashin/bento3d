...
export const RangeSlider: React.FC<RangeSliderProps> = ({max,min,label}) => {
  ...
  return (
    <>
    <div
      className='fixed inset-y-0 flex flex-col justify-center z-10 w-4 right-0'
    >
      <div 
          className={`w-4 flex flex-col items-end`}
          style={{ height: `${rulerRange*2}px`,gap:`${rulerRange*2/(length-1)}px` }}
          ref={unitRef}
      >
          {/* create span as many as length */}
          {[...Array(length)].map((_, index) => (
            <span key={index} className={`block h-1 ${index % 10 === 0 ?'w-6':index % 5 === 0 ? 'w-4' : 'w-2'} border-b border-black`}></span>
          ))}
      </div>
    </div>
