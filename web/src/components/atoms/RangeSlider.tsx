...
export const RangeSlider: React.FC<RangeSliderProps> = ({
  ...
  const type: number | null =
    label === "Height" ? 0 : label === "Depth" ? 1 : "Width" ? 2 : null;
  ...
  const calculateYPos = (currentY: number) => {
    ...
    const mappedValue = Math.round(
      ((newYPos + rulerRange) / (rulerRange * 2)) * (max - min),
    );
  ...
  return (
    <>
      <div
        className={
          type === 0
            ? "group fixed top-1/2 right-32 -translate-y-1/2 h-14 transition"
            : type === 1
              ? "group fixed top-1/2 left-32 -translate-y-1/2 h-14 transition"
              : type === 2
                ? "group fixed left-1/2 bottom-32 -translate-x-1/2 w-14 transition"
                : "invisible"
        }
      >
        <div
          className={
            type === 0
              ? "fixed inset-y-0 flex flex-col justify-center z-10 w-4 -right-32 translate-x-[500%] group-hover:translate-x-0 transition-all" //Height
              : type === 1
                ? "fixed inset-y-0 flex flex-col justify-center z-10 w-4 left-0 transition-all" //Depth
                : type === 2
                  ? "fixed inset-x-0 flex flex-row justify-center z-10 h-4 bottom-0 transition-all"
                  : "invisible"
          } //Width
        >
          <div
            className={
              type === 0
                ? `w-4 flex flex-col items-end`
                : type === 1
                  ? `w-4 flex flex-col items-start`
                  : type === 2
                    ? `h-4 flex flex-row items-end`
                    : "invisible"
            }
            style={{
              height: `${type === 0 || 1 ? rulerRange * 2 : type === 2 ? 16 : 0}px`,
              width: `${type === 0 || 1 ? 16 : type === 2 ? rulerRange * 2 : 0}px`,
            }}
            ref={unitRef}
          >
            ...
            {[...Array(length + 1)].map((_, index) => (
              <div
                ...
                className={
                  type === 0 || 1
                    ? `relative block min-h-[1px] ${index % 10 === 0 ? "w-6" : index % 5 === 0 ? "w-4" : "w-2"} bg-content-h-a`
                    : type === 2
                      ? `relative block w-[1px] ${index % 10 === 0 ? "h-6" : index % 5 === 0 ? "h-4" : "h-2"} bg-content-h-a`
                      : ""
                }
                style={{
              ...
              >
                {index % 10 === 0 && (
                  <p className="absolute text-sm text-right top-1/2 -translate-y-1/2 -left-8">
                    {index}
                  </p>
                )}
              </div>
        ...
        />
        <p className="absolute inset-0 leading-[56px] text-center h-full text-white text-base pointer-events-none">
          {label}
        </p>
      </div>
