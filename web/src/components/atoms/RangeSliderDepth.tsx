...
export const RangeSliderDepth: React.FC<RangeSliderDepthProps> = ({
  max,
  min,
  label,
}) => {
  ...
  return (
    <>
      <div
        className={
          "group fixed top-3/4 lg:top-1/2 lg:left-16 left-8 -translate-y-1/2 h-14 transition font-display"
        }
      >
        <div
          className={
            "fixed inset-y-0 flex flex-col justify-center z-10 w-4 lg:-left-16 -left-8 -translate-x-[500%] group-hover:translate-x-0 transition-all"
          } //
        >
          <div
          ...
          >
            ...
            {[...Array(length + 1)].map((_, index) => (
              ...
              >
                {index % 10 === 0 && (
                  <p className="absolute text-overline text-left top-1/2 -translate-y-1/2 -right-6">
                    {min + index}
                  </p>
                )}
              </div>
