...
export const RangeSliderWidth: React.FC<RangeSliderWidthProps> = ({
  max,
  min,
  label,
}) => {
  ...
  return (
    <>
      <div
        className={
          "group fixed left-1/2 bottom-16 -translate-x-1/2 w-[128px] transition font-display"
        }
      >
        <div
          className={
            "fixed inset-x-0 flex flex-row justify-center h-4 -bottom-16 translate-y-[500%] group-hover:translate-y-0 transition-all"
          }
        >
          <div
          ...
          >
            ...
            {[...Array(length + 1)].map((_, index) => (
              <div
                key={index}
                className={`relative block min-w-[1px] ${index % 10 === 0 ? "h-6" : index % 5 === 0 ? "h-4" : "h-2"} bg-content-h-a`}
                style={{
                  marginRight: `${(rulerRange * 2 - length) / (length - 1)}px`,
                }}
              >
                {index % 10 === 0 && (
                  <p className="absolute text-overline text-center left-1/2 -translate-x-1/2 -top-4">
                    {min + index}
                  </p>
                )}
              </div>
        ...
        <Icon
          name="chevronRight"
          className="absolute -right-6 h-full w-4 group-hover:-right-8 transition-all"
        />
        <Icon
          name="chevronLeft"
          className="absolute -left-6 h-full w-4 group-hover:-left-8 transition-all"
        />
