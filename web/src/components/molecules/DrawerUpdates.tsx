...
import { isUpdatesDrawerOpenAtom } from "../../store/index";
import { Drawer } from "../atoms/Drawer";
import updates from "../../assets/jsons/updates.json";
import { Icon } from "../atoms/Icon";

interface DrawerUpdatesProps {
}
...
export const DialogFeedback: FC<DrawerUpdatesProps> = ({}) => {
  const [isOpen, setIsOpen] = useAtom(isUpdatesDrawerOpenAtom);
  const sortedUpdates = updates.sort((a: Update, b: Update) => {
    return new Date(b.date).getTime() - new Date(a.date).getTime();
  });

  const groupedUpdates = sortedUpdates.reduce((acc: { [key: string]: Update[] }, update: Update) => {
    const month = new Date(update.date).toLocaleString('en-US', { month: 'long', year: 'numeric' });
    if (!acc[month]) {
      acc[month] = [];
    }
  ...
  return (
    <Drawer isOpen={isOpen} onClose={() => setIsOpen(false)} className="max-w-sm bg-transparent overflow-y-auto">
      <div className="w-full h-full p-8">
        <h3 className="text-center mb-6 font-display text-xl">UPDATEs</h3>
        <p className="mb-8 text-content-m-a text-center text-sm">Bento3D keeps evolving, <br/>making your 3D printing life even better.</p>
          ...
          {Object.entries(groupedUpdates).map(([month, updates], index) => (
            <div key={index} className="grid grid-cols-[auto_1fr] gap-2">
                ...
                <div>
                    ...
                    <h4 className="font-bold text-content-h text-left mb-2 h-6">{month}</h4>
                    <ul>
                        {updates.map((update: Update, idx: number) => (
                        <li key={idx} className={`mb-4 bg-content-h rounded-sm shadow-md relative ${update.image?'p-0':'p-4'} overflow-hidden`}>
                            <h5 className={`font-bold ${update.image?'absolute':'relative'} ${update.style?update.style:'left-4 bottom-4 text-white'}`}>{update.title}</h5>
