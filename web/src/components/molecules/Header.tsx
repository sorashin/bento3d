...
import { isFeedbackDialogOpenAtom, isSettingDialogOpenAtom, isUpdatesDrawerOpenAtom, stepSyncAtom } from "../../store";
...
export const Header = () => {
    ...
    const [isUpdatesDrawerOpen,setIsUpdatesDrawerOpen] = useAtom(isUpdatesDrawerOpenAtom)
    const latestUpdate = updates.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime())[0];
    const isNew = new Date().getTime() - new Date(latestUpdate.date).getTime() <= 7 * 24 * 60 * 60 * 1000;
        ...
        <header className="absolute inset-x-0 top-0 pt-8 px-4 z-20 flex flex-col justify-between">
            ...
            <div className="flex justify-between items-center gap-2 md:mt-0 md:absolute md:left-8">
                <h1 className="flex gap-1 items-center">
                    ...
                    <Icon name="chevronRight" className="size-4 text-content-xxl-a stroke-2"/>
                </h1>
                <button className="b-button bg-transparent" onClick={()=>setIsFeedbackDialogOpen(true)}
                        data-tooltip-content={"Feedback"}
                        data-tooltip-id={"hint-tooltip"}
                    >💬</button>
                <button className="b-button bg-transparent" onClick={()=>setIsUpdatesDrawerOpen(true)}
                        data-tooltip-content={"Updates"}
                        data-tooltip-id={"hint-tooltip"}
                    >📣{isNew && <span className="size-2 rounded-full bg-system-error-h absolute bottom-1.5 right-1.5 border-spacing-1 border-system-error-l"></span>}</button>
            </div>
