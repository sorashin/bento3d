...
const Menu: FC<{
  ...
  onClick: () => void;
  isOpen: boolean;
}> = ({ children, index, onClick, isOpen, title }) => {
  return (
...
export const ConfigView: FC<ConfigViewProps> = () => {
  ...
  const [isDialogDownloadOpen, setIsDialogDownloadOpen] = useAtom(
    isDownloadDialogOpenAtom,
  );
  ...
  useEffect(() => {
    setOpenList(() => {
      return [...Array(title.length)].map((v, i) =>
        i === boxConfig.viewMode ? true : false,
      );
    });
  }, [boxConfig.viewMode]);
