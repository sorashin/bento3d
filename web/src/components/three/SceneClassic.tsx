              ...
              <>
                {!(
                  ...
                  <primitive object={element} key={element.uuid}>
                    ...
                    {index === 3 && (
                      <meshStandardMaterial
                        color={`${colorPalette[boxConfig.colorMode].secondary}`}
                      />
                    )}
                  </primitive>
                )}
...
interface SceneClassicProps {
  group: THREE.Group;
}

export const SceneClassic: React.FC<SceneClassicProps> = ({ group }) => {
  return (
